...
class FT_Lift(torch.nn.Module):
    def __init__(self, in_channels, latent_channels, model, pre_lift=False):
        ...
        if pre_lift:
            self.linear1 = torch.nn.Conv2d(
                in_channels=in_channels, out_channels=128, kernel_size=1, padding=0
            )

            self.linear2 = torch.nn.Conv2d(
                in_channels=128, out_channels=latent_channels, kernel_size=1, padding=0
            )
        else:
            self.linear1 = torch.nn.Conv2d(
...
class FT_Project(torch.nn.Module):
    def __init__(self, new_out_dim, model, init_project=False):
        super(FT_Project, self).__init__()

        if init_project:
            self.in_channels = (
                model.encoder_features[0] + model.decoder_features_out[-1]
            )
            ...
            self.post_linear1 = torch.nn.Conv2d(
                in_channels=new_out_dim, out_channels=128, kernel_size=1, padding=0
            )
            self.post_linear2 = torch.nn.Conv2d(
                in_channels=128, out_channels=new_out_dim, kernel_size=1, padding=0
            )
...
def initialize_FT(model, old_in_dim, new_in_dim, new_out_dim, old_out_dim):
    ...
    model.project = FT_Project(
        new_out_dim=new_out_dim, model=model, init_project=init_project
    )
