...
def upsample2(u, N, fourier=False):
    ...
    if d == 2:
        ...
        u_hat_up = u_hat_up.at[:, :, : N_old // 2 + 1, : N_old // 2 + 1].set(
            u_hat[:, :, : N_old // 2 + 1, : N_old // 2 + 1]
        )
        u_hat_up = u_hat_up.at[:, :, : N_old // 2 + 1, -N_old // 2 :].set(
            u_hat[:, :, : N_old // 2 + 1, -N_old // 2 :]
        )
        u_hat_up = u_hat_up.at[:, :, -N_old // 2 :, : N_old // 2 + 1].set(
            u_hat[:, :, -N_old // 2 :, : N_old // 2 + 1]
        )
        u_hat_up = u_hat_up.at[:, :, -N_old // 2 :, -N_old // 2 :].set(
            u_hat[:, :, -N_old // 2 :, -N_old // 2 :]
        )
        """elif d == 3:
...
def computer_relative_error(qoi, approx_qoi, order=2):
    ...
    relative_error = (
        jnp.mean(jnp.abs(approx_qoi - qoi) ** order) / jnp.mean(jnp.abs(qoi) ** order)
    ) ** (1 / order)
