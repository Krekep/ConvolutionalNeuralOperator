...
class CNOBlock(nn.Module):
    def __init__(
        ...
        if activation == "cno_lrelu":
            ...
            self.activation = LReLu_standard(
                in_channels=self.out_channels,
                out_channels=self.out_channels,
                in_size=self.in_size,
                out_size=self.out_size,
        ...
        if is_time == 1 or is_time == True:
            self.time_steps = time_steps
            self.in_norm_conditiner = FILM(out_channels, dim=nl_dim, s=self.in_size)
            self.batch_norm = nn.Identity()
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        if self.is_time == 1 or self.is_time == True:
            self.time_steps = time_steps
            self.in_norm_conditiner1 = FILM(channels, dim=nl_dim, s=self.size)
            self.in_norm_conditiner2 = FILM(channels, dim=nl_dim, s=self.size)
            self.batch_norm1 = nn.Identity()
            self.batch_norm2 = nn.Identity()
...
class CNO_time(pl.LightningModule):
    def __init__(
        ...
        if not is_att:
            ...
            _dim = int(
                dim_multiplier * patch_size**2 * self.encoder_features[self.N_layers]
            )
            self.transformer = ViT(
                image_size=self.encoder_sizes[self.N_layers],
        ...
        self.loader_dictionary = (
            loader_dictionary  # IMPORTANT -- Experiment and Training parameters
        )
        ...
        if ("separate" in self.loader_dictionary) and self.loader_dictionary[
            "separate"
        ]:
    ...
    def forward(self, x, time):
        ...
        for i in range(self.N_layers):
            ...
            if i == 0:
                ...
                x = torch.cat(
                    (x, self.ED_expansion[self.N_layers - i](skip[-i], time)), 1
                )
    ...
    def training_step(self, batch):
        ...
        is_separate = (
            ("separate" in self.loader_dictionary)
            and self.loader_dictionary["separate"]
            and "separate_dim" in self.loader_dictionary
        )
    ...
    def configure_optimizers(self):
        ...
        if (
            "fine_tuned" in self.loader_dictionary
            and self.loader_dictionary["fine_tuned"]
        ):
            ...
            params_1 = [
                ...
                and ("lift" not in name)
                and ("in_norm_conditiner" not in name)
            ]

            params_2 = [
                param
                for name, param in self.named_parameters()
                if (("project" in name) or ("lift" in name))
                and ("in_norm_conditiner" not in name)
            ]
            ...
            optimizer = torch.optim.AdamW(
                [
                    {"params": params_1},
                    {"params": params_2, "lr": self.lr_emb},
                    {"params": params_3, "lr": self.lr_norm},
                ],
                lr=self.lr,
                weight_decay=self.loader_dictionary["weight_decay"],
            )
        else:
            optimizer = torch.optim.AdamW(
    ...
    def train_dataloader(self):
        ...
        if which == "eul_ns_mix1":
            ...
            is_masked = (
                "is_masked" in self.loader_dictionary
                and self.loader_dictionary["is_masked"] is not None
            )
            if is_masked:
                if which[:2] == "ns":
        ...
        train_loader = DataLoader(
            train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=6
        )
        return train_loader
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # ---------------
        if not is_masked:
            # loss = nn.L1Loss()(output_pred_batch, output_batch)
            loss = nn.L1Loss()(output_pred_batch, output_batch)
        ...
        if batch_idx == 0:
            ...
            self.validation_errs[str(dataloader_idx)] = torch.cat(
                (self.validation_errs[str(dataloader_idx)], loss)
            )

            if is_separate:
                for i in range(self.num_separate):
                    self.validation_errs_sep[str(dataloader_idx)][i] = torch.cat(
                        (self.validation_errs_sep[str(dataloader_idx)][i], loss_sep[i])
                    )
    ...
    def on_validation_epoch_end(self):
        ...
        return {
            "med_val_l": median_loss,
            "mean_val_l": mean_loss,
        }
