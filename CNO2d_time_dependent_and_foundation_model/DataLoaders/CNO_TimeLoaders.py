...
class RichtmyerMeshkov(BaseTimeDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.tracer:
            input_tracer = (
                torch.from_numpy(self.reader["solution"][i + self.start, t1, 4:5])
                .type(torch.float32)
                .reshape(1, self.resolution, self.resolution)
            )
            output_tracer = (
                torch.from_numpy(self.reader["solution"][i + self.start, t2, 4:5])
                .type(torch.float32)
                .reshape(1, self.resolution, self.resolution)
            )
...
class PiezoConductivityNoCondition(BaseTimeDataset):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert self.max_num_time_steps * self.time_step_size <= 20
