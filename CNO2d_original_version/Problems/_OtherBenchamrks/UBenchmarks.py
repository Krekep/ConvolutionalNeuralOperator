        ...
        for i in range(n_samples):
            ...
            if self.in_size < 64:
                input_data[i, 0] = downsample(
                    np.array(f.variables["u"][:]).reshape(1, 1, 64, 64), self.in_size
                )
            else:
...
class ShearLayerRecursive:
    ...
    def get_data(self, folder, n_samples):
        input_data = np.zeros(
            (5 * (n_samples + 64), 1 + 2 * self.N_Fourier_F, self.in_size, self.in_size)
        )
...
class Translation:
    ...
    def get_data(self, file, n_samples, device):
        ...
        input_data = (
            torch.tensor(np.zeros((n_samples, 1 + 2 * self.N_Fourier_F, 64, 64)))
            .type(torch.float32)
            .to(device)
        )
        output_data = (
    ...
    def get_data(self, file, n_samples):
        ...
        with h5py.File(file, "r") as f:
            ...
            for i in range(n_samples):
                ...
                input_data[i] = torch.tensor(np.array(f[a_group_key[0]][s_in])).reshape(
                    1, 32, 32
                )
...
class NEMO:
    def __init__(
        self, network_properties, device, batch_size, cluster=True, notebook=False
    ):
        ...
        self.train_loader = DataLoader(
            TensorDataset(train_a, train_u), batch_size=batch_size, shuffle=True
        )
        self.test_loader = DataLoader(
