...
class ShearLayerDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.s == 64:
            ...
            inp = self.reader["Sample_" + str(index + self.start)]["input"][:].reshape(
                self.s, self.s
            )
            ...
            inputs = (
                torch.from_numpy(inp).type(torch.float32).reshape(1, self.s, self.s)
            )
            labels = (
                torch.from_numpy(
            ...
            file_input = (
                self.folder + "sample_" + str(index + self.start) + "_time_0.nc"
            )
            file_output = (
                self.folder + "sample_" + str(index + self.start) + "_time_10.nc"
            )

            f = netCDF4.Dataset(file_input, "r")
            if self.s < 128:
                inputs = downsample(
                    np.array(f.variables["u"][:]).reshape(1, 1, 128, 128), self.s
                ).reshape(1, self.s, self.s)
