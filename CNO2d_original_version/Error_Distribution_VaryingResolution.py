    ...
    with torch.no_grad():
        for i, (inputs, outputs) in enumerate(testing_loader):
            ...
            if which_model == "CNN":
                ...
                err = (
                    torch.mean(
                        abs(prediction[:, 0, :, :] - outputs[:, 0, :, :]) ** p, (-2, -1)
                    )
...
def plot_samples(
    which_model, data_loader, model, p, n, s, s_train, cmap="jet", which="shear_layer"
):
...
def plot_different_resolution(D, YTicks, title, loc_legend="upper right"):
    ...
    for k in D:
        ...
        if first:
            ...
            axes.scatter(
                [_t_res], [E[_index_t]], color="red", marker="o", zorder=5, s=75
            )
