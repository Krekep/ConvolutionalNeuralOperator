...
class ResNetBlock(nn.Module):
    def __init__(self, in_channels, kernel_size=3):  # Number of input channels.
...
class Conv2D(nn.Module):
    def __init__(
        ...
        self.kernel_size = kernel_size
        self.cont_conv_layers = nn.ModuleList(
            [
                nn.Conv2d(
                    self.feature_maps[i],
        ...
        self.cont_conv_layers_invariant = nn.ModuleList(
            [
                nn.Conv2d(
        ...
        for i in range(N_res):
            self.resnet_blocks.append(
                ResNetBlock(in_channels=self.feature_maps[self.N_layers // 2])
            )
    ...
    def forward(self, x):
        ...
        for i in range(self.N_layers // 2, self.N_layers):
            x = self.downsample2(
                self.sigma(self.upsample2(self.cont_conv_layers_invariant[i](x)))
            )
            # print("AFTER INV",x.shape)

            x = self.downsample2(
                self.sigma(self.upsample4(self.cont_conv_layers[i](x)))
            )
...
class ConvBranch2D(nn.Module):
    def __init__(
        ...
        for i in range(N_res):
            self.resnet_blocks.append(
                ResNetBlock(in_channels=self.feature_maps[self.N_layers])
            )
